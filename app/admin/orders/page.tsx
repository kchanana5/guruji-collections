...
export default async function AdminOrdersPage() {
  ...
  return <main className="min-h-screen bg-[#f7f3ec] px-5 py-8 text-[#171717] sm:px-8 lg:px-12"><div className="mx-auto max-w-7xl">
    ...
    <section className="mt-8 overflow-hidden rounded-3xl border border-black/10 bg-white"><div className="overflow-x-auto"><table className="w-full min-w-[1050px] text-left text-sm"><thead className="border-b bg-black/[0.02] text-xs uppercase tracking-wide text-black/45"><tr><th className="px-5 py-4">Order</th><th className="px-5 py-4">Customer</th><th className="px-5 py-4">Payment</th><th className="px-5 py-4">Amount</th><th className="px-5 py-4">Status</th><th className="px-5 py-4">Shipment</th><th className="px-5 py-4">Action</th></tr></thead><tbody>{(orders ?? []).map((o: any) => { const a=o.shipping_address||{}; const p=Array.isArray(o.payments)?o.payments[0]:o.payments; const s=Array.isArray(o.shipments)?o.shipments[0]:o.shipments; return <tr key={o.id} className="border-b last:border-0"><td className="px-5 py-5"><Link href={`/admin/orders/${o.id}`} className="font-bold underline-offset-4 hover:underline">{o.order_number}</Link><p className="mt-1 text-xs text-black/45">{new Date(o.created_at).toLocaleString("en-IN")}</p></td><td className="px-5 py-5"><p className="font-semibold">{a.recipientName || "Customer"}</p><p className="text-xs text-black/45">{a.city || ""} {a.postalCode || ""}</p></td><td className="px-5 py-5"><span className="rounded-full bg-black/[0.05] px-3 py-1 text-xs font-semibold">{p?.status || "pending"}</span></td><td className="px-5 py-5 font-bold">₹{Number(o.grand_total).toLocaleString("en-IN")}</td><td className="px-5 py-5"><span className="rounded-full bg-black/[0.05] px-3 py-1 text-xs font-semibold">{o.status}</span></td><td className="px-5 py-5">{s?.awb_code ? <div><p className="font-semibold">{s.courier_name || "Courier"}</p><a className="text-xs underline" href={s.tracking_url || `https://shiprocket.co/tracking/${s.awb_code}`} target="_blank" rel="noreferrer">AWB {s.awb_code}</a></div> : <span className="text-xs text-black/45">Ready for shipment</span>}</td><td className="px-5 py-5"><Link href={`/admin/orders/${o.id}`} className="rounded-lg border border-black/10 px-3 py-2 text-xs font-bold">Open</Link></td></tr> })}</tbody></table>{!orders?.length && <div className="p-12 text-center text-sm text-black/50">No orders yet. Your first paid GJC order will appear here.</div>}</div></section>
  </div></main>;
}
