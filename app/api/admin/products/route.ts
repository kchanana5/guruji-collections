...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ error: "Invalid AI draft data." }, { status: 400 });
  }
  ...
  if (!draft?.name || !Number.isFinite(price) || price < 0) {
    return NextResponse.json({ error: "Product title and a valid INR price are required." }, { status: 400 });
  }
  if (publish && !(image instanceof File)) {
    return NextResponse.json({ error: "A product image is required before publishing." }, { status: 400 });
  }
  ...
  if (image instanceof File && image.size > 6 * 1024 * 1024) {
    return NextResponse.json({ error: "Please choose an image smaller than 6 MB." }, { status: 400 });
  }
  if (publish && !variants.length) {
    return NextResponse.json({ error: "Add at least one size/colour variant before publishing." }, { status: 400 });
  }
  ...
  for (const variant of variants) {
    ...
    if (!sku || !size || !color || !Number.isInteger(stock) || stock < 0 || (variantPrice !== null && (!Number.isFinite(variantPrice) || variantPrice < 0))) {
      return NextResponse.json({ error: "Every variant needs a size, colour, SKU and valid stock/price." }, { status: 400 });
    }
    if (seenSkus.has(sku)) return NextResponse.json({ error: `Duplicate SKU: ${sku}` }, { status: 400 });
    seenSkus.add(sku);
  }
  if (publish && !variants.some((variant: any) => Number(variant?.stock_quantity) > 0)) {
    return NextResponse.json({ error: "Add stock to at least one variant before publishing." }, { status: 400 });
  }
  ...
  try {
    ...
    if (image instanceof File) {
      uploadedPath = `${product.id}/${Date.now()}-${safeFileName(image.name)}`;
      const upload = await supabase.storage.from("product-images").upload(uploadedPath, image, {
      ...
      const { error: imageError } = await supabase.from("product_images").insert({
        ...
        alt_text: String(draft.name || "GJC product"),
      });
      if (imageError) throw new Error(imageError.message);
    }

    return NextResponse.json({ id: product.id, slug: product.slug, published: publish });
  } catch (error) {
    if (uploadedPath) {
      await supabase.storage.from("product-images").remove([uploadedPath]).catch(() => undefined);
    }
